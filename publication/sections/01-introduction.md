...
This paper presents POMASA (Pattern-Oriented Multi-Agent System Architecture), a pattern language for constructing declarative multi-agent systems. POMASA is neither a framework (it imposes no mandatory code structure) nor a tool (the generator we provide is merely one way to apply it). At its core, POMASA is a pattern language[^3]—a network of interconnected design patterns that guide both humans and AI in building MAS.
...
[^3]: Alexander, C., 1977. *A Pattern Language: Towns, Buildings, Construction*. Oxford University Press.
